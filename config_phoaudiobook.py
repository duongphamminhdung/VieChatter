...
class PhoAudiobookConfig:
    # --- Paths ---
    base_dir: str = "/content/drive/MyDrive/VietP"

    # Directory where setup.py downloaded the pretrained models (in Drive)
    model_dir: str = os.path.join(base_dir, "pretrained_models")
    ...
    # Using /content/ for Colab (faster than Drive)
    preprocessed_dir: str = os.path.join(base_dir, "phoaudiobook_preprocessed")

    # Output directory for finetuned model - Save to Google Drive
    output_dir: str = os.path.join(base_dir, "models")
    ...
    resume_from_checkpoint: bool = True  # Set True to resume from latest checkpoint
