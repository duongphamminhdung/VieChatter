...
Automatically detects and uses preprocessed data if available, otherwise processes on-the-fly.

Use this script for all training scenarios - no need to choose!
"""
...
# Internal Modules
from src.config import PhoAudiobookConfig
...
class PreprocessedDataset(torch.utils.data.Dataset):
    """Dataset that loads from single preprocessed .pt file."""
    ...
    def __init__(self, preprocessed_file):
        ...
        # Extract components
        self.speech_tokens = self.data['speech_tokens'].long()
        self.speaker_emb = self.data['speaker_emb'].float()
        self.prompt_tokens = self.data['prompt_tokens'].long()
        self.text_tokens = self.data['text_tokens'].long()
        self.speaker_ids = self.data['speaker_ids'] if 'speaker_ids' in self.data else [f"unk_{i}" for i in range(len(self.speech_tokens))]
        
        logger.info(f"Loaded {len(self.speech_tokens)} preprocessed samples")
        
    def __len__(self):
        return len(self.speech_tokens)
    
    def __getitem__(self, idx):
        """Load a preprocessed sample."""
        return {
            'speech_tokens': self.speech_tokens[idx],
...
def data_collator_preprocessed(batch):
    """Collate preprocessed samples into batches with padding and lengths."""
    ...
    # Pad sequences (preprocessed data may have different lengths)
    ...
    prompt_tokens = pad_sequence([item['prompt_tokens'] for item in batch], batch_first=True, padding_value=0)
    
    # Stack speaker embeddings (these should all be same size)
    speaker_emb = torch.stack([item['speaker_emb'] for item in batch])
    
    # Calculate lengths (required for masking in training)
    text_token_lens = torch.tensor([len(item['text_tokens']) for item in batch], dtype=torch.long)
    speech_token_lens = torch.tensor([len(item['speech_tokens']) for item in batch], dtype=torch.long)
...
def main():
    ...
    preprocessed_file = os.path.join(cfg.preprocessed_dir, "preprocessed_all.pt")
    
    if os.path.exists(preprocessed_file):
        # Use preprocessed data
        logger.info("✓ Preprocessed data found!")
        logger.info(f"  Loading from: {preprocessed_file}")
        logger.info(f"  Using preprocessed dataset (faster training)")
        
        train_ds = PreprocessedDataset(preprocessed_file)
        num_train_samples = len(train_ds)
        
        logger.info(f"Loaded {num_train_samples} preprocessed samples")
        
        # Use preprocessed collator
        ...
        # Create file-based dataset (expects preprocessed .pt files)
        train_ds = ChatterboxDataset(
            config=cfg,
            num_samples=cfg.max_samples if hasattr(cfg, 'max_samples') else None
        )
    ...
    logger.info(f"Using 4 workers for parallel processing within each partition")
    logger.info("=" * 70)
